...
--1. Select details of all patients aged less than 18 years on the visit date who were seen in more than
--one department in calendar year 2017. The result set should have ONLY one record for each
--patient and department combination.
WITH PatientDepartmentVisits AS (
    SELECT
        P.PATIENT_ID,
        P.PATIENT_NAME,
        P.BIRTH_DATE,
        D.DEPT_NAME AS DEPARTMENT_NAME,
        COUNT(T.PATIENT_ID) AS VISIT_COUNT
    FROM
        TBL_PATIENTS P
    INNER JOIN
        TBL_TRANSACTIONS T ON P.PATIENT_ID = T.PATIENT_ID
    INNER JOIN
        TBL_PHYSICIANS PH ON T.PHYSICIAN_ID = PH.PHYSICIAN_ID
    INNER JOIN
        TBL_DEPARTMENTS D ON PH.DEPT_ID = D.DEPT_ID
    WHERE
        P.BIRTH_DATE > DATEADD(YEAR, -18, GETDATE()) -- Patients aged less than 18
        AND YEAR(T.VISIT_DATE) = 2017
    GROUP BY
        P.PATIENT_ID,
        P.PATIENT_NAME,
        P.BIRTH_DATE,
...
SELECT
    ...
    VISIT_COUNT
FROM
    PatientDepartmentVisits;



-- 2. Select details of all patients with outstanding balance (CHARGES > INSURANCE_PAYMENT +
-- PATIENT_PAYMENT) in each department, sorted DESCENDING by outstanding balance. Display
-- Insurance as 'SELF-PAY' where INSURANCE_ID is blank.
WITH OutstandingPatients AS (
    SELECT
        D.DEPT_NAME,
        P.PATIENT_ID,
        P.PATIENT_NAME,
        COALESCE(I.INSURANCE_NAME, 'SELF-PAY') AS INSURANCE_NAME,
        SUM(T.CHARGES - COALESCE(T.INSURANCE_PAYMENT, 0) - COALESCE(T.PATIENT_PAYMENT, 0)) AS TOTAL_OUTSTANDING_BALANCE
    FROM
        TBL_PATIENTS P
    INNER JOIN
        TBL_TRANSACTIONS T ON P.PATIENT_ID = T.PATIENT_ID
    ...
    INNER JOIN
        TBL_DEPARTMENTS D ON PH.DEPT_ID = D.DEPT_ID
    LEFT JOIN
        TBL_INSURANCES I ON P.INSURANCE_ID = I.INSURANCE_ID
    WHERE
        T.CHARGES > COALESCE(T.INSURANCE_PAYMENT, 0) + COALESCE(T.PATIENT_PAYMENT, 0)
    GROUP BY
        D.DEPT_NAME,
        P.PATIENT_ID,
        P.PATIENT_NAME,
        COALESCE(I.INSURANCE_NAME, 'SELF-PAY')
)
SELECT
    DEPT_NAME,
    PATIENT_ID,
...
-- 3. Rank physicians who served the patients with highest total insurance_payment for the insurance
--providers having lowest total outstanding balance. Select and show only 2nd and 3rd ranked
--physicians name and department name.

WITH PhysicianRanking AS (
    SELECT
        PH.PHYSICIAN_NAME,
        D.DEPT_NAME,
        RANK() OVER (ORDER BY SUM(T.INSURANCE_PAYMENT) DESC) AS InsuranceRank,
        RANK() OVER (ORDER BY SUM(T.CHARGES - COALESCE(T.INSURANCE_PAYMENT, 0) - COALESCE(T.PATIENT_PAYMENT, 0)) ASC) AS OutstandingBalanceRank
    FROM
        TBL_PHYSICIANS PH
    INNER JOIN
        TBL_TRANSACTIONS T ON PH.PHYSICIAN_ID = T.PHYSICIAN_ID
    INNER JOIN
        TBL_DEPARTMENTS D ON PH.DEPT_ID = D.DEPT_ID
    INNER JOIN
        TBL_PATIENTS P ON T.PATIENT_ID = P.PATIENT_ID
    WHERE
        T.INSURANCE_PAYMENT IS NOT NULL
    GROUP BY
        PH.PHYSICIAN_NAME, D.DEPT_NAME
)
SELECT
    PHYSICIAN_NAME,
    DEPT_NAME
FROM
    PhysicianRanking
WHERE
    InsuranceRank IN (2, 3);




-- 4. Select all transactions with visit dates in calendar year 2016 and 2017. Display 
-- Insurance as 'SELFPAY' where INSURANCE_ID is blank. Present the information in an Excel 
-- pivot table organized by visit month and financial class. Display the total payment 
-- (Insurance Payment + Patient Payment)values in the pivot table.

-- CTE to retrieve transactions with visit dates in calendar year 2016 and 2017
WITH SelectedTransactions AS (
    SELECT
        MONTH(T.VISIT_DATE) AS VisitMonth,
        IIF(I.INSURANCE_ID IS NULL, 'SELF-PAY', I.INSURANCE_NAME) AS FinancialClass,
        COALESCE(T.INSURANCE_PAYMENT, 0) + COALESCE(T.PATIENT_PAYMENT, 0) AS TotalPayment
    FROM
        TBL_TRANSACTIONS T
    LEFT JOIN
        TBL_PATIENTS P ON T.PATIENT_ID = P.PATIENT_ID
    LEFT JOIN
        TBL_INSURANCES I ON P.INSURANCE_ID = I.INSURANCE_ID
    WHERE
        YEAR(T.VISIT_DATE) IN (2016, 2017)
)

-- Query to retrieve the data for Excel pivot table
SELECT
    VisitMonth,
    FinancialClass,
...
GROUP BY
    VisitMonth, FinancialClass
ORDER BY
    VisitMonth, FinancialClass;



-- 5.Identify any anomalies and find the TX_IDs that are causing these.
...
SELECT
    T.TX_ID,
    MONTH(T.VISIT_DATE) AS VISIT_MONTH,
    T.INSURANCE_NAME,
    T.INSURANCE_PAYMENT,
    T.PATIENT_PAYMENT,
    (T.INSURANCE_PAYMENT + T.PATIENT_PAYMENT) AS TOTAL_PAYMENT
FROM
    TransactionDetails T
WHERE
    (T.INSURANCE_PAYMENT + T.PATIENT_PAYMENT) >= 10000
ORDER BY
    VISIT_MONTH, T.INSURANCE_NAME, T.TX_ID;
